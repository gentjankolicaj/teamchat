...
-- Table `teamchat`.`chat`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `teamchat`.`chat` ;

CREATE TABLE IF NOT EXISTS `teamchat`.`chat` (
  `id` INT NOT NULL,
...
CREATE INDEX `fk_chat_user1_idx` ON `teamchat`.`chat` (`sender` ASC) VISIBLE;

CREATE INDEX `fk_chat_team1_idx` ON `teamchat`.`chat` (`team_id` ASC) VISIBLE;

CREATE INDEX `fk_chat_user2_idx` ON `teamchat`.`chat` (`receiver` ASC) VISIBLE;
